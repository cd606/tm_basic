#ifndef TM_KIT_BASIC_TOP_DOWN_SINGLE_PASS_ITERATION_CLOCK_CLOCK_ON_ORDER_FACILITY_HPP_
#define TM_KIT_BASIC_TOP_DOWN_SINGLE_PASS_ITERATION_CLOCK_CLOCK_ON_ORDER_FACILITY_HPP_
                ...
                virtual void handle(typename M::template InnerData<typename M::template Key<FacilityInput<S>>> &&input) override final {
                    ...
                    if (filteredDurations.empty()) {
                        if constexpr (simple_callback_checker((F *) nullptr)) {
                            ...
                            throw std::runtime_error("top_down_single_pass_iteration_clock::ClockFacility::createGenericClockCallback: bad function type");
                        }                        
                    } else {
                        std::sort(filteredDurations.begin(), filteredDurations.end());
                        std::size_t count = filteredDurations.size();
