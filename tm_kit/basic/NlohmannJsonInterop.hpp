...
namespace dev { namespace cd606 { namespace tm { namespace basic { namespace nlohmann_json_interop {
    ...
    struct JsonRequiredFieldMark {};
    template <class T>
    using JsonRequiredField = SingleLayerWrapperWithTypeMark<JsonRequiredFieldMark, T>;    
    template <class T>
    static constexpr bool IsJsonRequiredField = IsSingleLayerWrapperWithGivenTypeMark<JsonRequiredFieldMark, T>::Value;    

    struct JsonVariantTypeMark {};
    struct JsonVariantAutoDetectTypeMark {};

    template <class... Ts>
    using JsonVariant = SingleLayerWrapperWithTypeMark<JsonVariantTypeMark, std::variant<std::monostate,Ts...>>;
    template <class... Ts>
    using JsonVariantAutoDetect = SingleLayerWrapperWithTypeMark<JsonVariantAutoDetectTypeMark, std::variant<std::monostate,Ts...>>;

    template <class M, class T>
    class JsonEncoder<SingleLayerWrapperWithTypeMark<M,T>, std::enable_if_t<!std::is_same_v<M,JsonVariantTypeMark> && !std::is_same_v<M,JsonVariantAutoDetectTypeMark>, void>> {
    public:
    ...
    template <class M, class T>
    struct JsonWrappable<SingleLayerWrapperWithTypeMark<M,T>, std::enable_if_t<!std::is_same_v<M,JsonVariantTypeMark> && !std::is_same_v<M,JsonVariantAutoDetectTypeMark>, void>> {
        static constexpr bool value = JsonWrappable<T>::value;
    };
    ...
    template <class... Ts>
    struct JsonWrappable<JsonVariantAutoDetect<Ts...>, void> {
    private:
        template <std::size_t Index>
        static constexpr bool value_internal() {
            if constexpr (Index < sizeof...(Ts)) {
    ...
    template <class... Ts>
    class JsonEncoder<JsonVariantAutoDetect<Ts...>, std::enable_if_t<JsonWrappable<JsonVariantAutoDetect<Ts...>>::value, void>> {
    private:
        template <std::size_t Index>
        static void write_impl(nlohmann::json &output, JsonVariantAutoDetect<Ts...> const &data) {
            if constexpr (Index >= 0 && Index < sizeof...(Ts)) {
                if (Index+1 == data.value.index()) {
                    using F = std::variant_alternative_t<Index+1,std::variant<std::monostate,Ts...>>;
                    JsonEncoder<F>::write(
    ...
    public:
        static void write(nlohmann::json &output, std::optional<std::string> const &key, JsonVariantAutoDetect<Ts...> const &data) {
            if constexpr (sizeof...(Ts) >= 0) {
                write_impl<0>((key?output[*key]:output), data);
            }
        }
    ...
    template <class M, class T>
    class JsonDecoder<SingleLayerWrapperWithTypeMark<M,T>, std::enable_if_t<!std::is_same_v<M,JsonVariantTypeMark> && !std::is_same_v<M,JsonVariantAutoDetectTypeMark>, void>> {
    public:
        static void fillFieldNameMapping(JsonFieldMapping const &mapping=JsonFieldMapping {}) {
            JsonDecoder<T>::fillFieldNameMapping(mapping);
        }
    ...
    template <class... Ts>
    class JsonDecoder<JsonVariantAutoDetect<Ts...>, std::enable_if_t<JsonWrappable<JsonVariantAutoDetect<Ts...>>::value, void>> {
    private:
        template <std::size_t Index>
        static bool read_impl(nlohmann::json const &input, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping) {
            if constexpr (Index < sizeof...(Ts)) {
                using F = std::variant_alternative_t<Index+1, std::variant<std::monostate,Ts...>>;
                data.value.template emplace<Index+1>();
                bool res = JsonDecoder<F>::read(input, std::nullopt, std::get<Index+1>(data.value), mapping);
                if (res) {
                    return res;
                } else {
                    return read_impl<Index+1>(input, data, mapping);
                ...
                data.value.template emplace<0>();
                return false;
            }
        }
        template <std::size_t Index>
        static bool read_impl_simd(simdjson::dom::element const &input, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping) {
            if constexpr (Index < sizeof...(Ts)) {
                using F = std::variant_alternative_t<Index+1, std::variant<std::monostate,Ts...>>;
                data.value.template emplace<Index+1>();
                bool res = JsonDecoder<F>::read_simd(input, std::nullopt, std::get<Index+1>(data.value), mapping);
                if (res) {
                    return res;
                } else {
                    return read_impl_simd<Index+1>(input, data, mapping);
        ...
        template <std::size_t Index, class X>
        static bool read_impl_simd_ondemand(X &input, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping) {
            if constexpr (Index < sizeof...(Ts)) {
                using F = std::variant_alternative_t<Index+1, std::variant<std::monostate,Ts...>>;
                data.value.template emplace<Index+1>();
                bool res = JsonDecoder<F>::read_simd_ondemand(input, std::nullopt, std::get<Index+1>(data.value), mapping);
                if (res) {
                    return res;
                } else {
                    return read_impl_simd_ondemand<Index+1,X>(input, data, mapping);
    ...
    public:
        ...
        static bool read(nlohmann::json const &input, std::optional<std::string> const &key, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping=JsonFieldMapping {}) {
            auto const &i = (key?input.at(*key):input);
            return read_impl<0>(i, data, mapping);
        }
        static bool read_simd(simdjson::dom::element const &input, std::optional<std::string> const &key, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping=JsonFieldMapping {}) {
            if (key) {
                if (input[*key].error() == simdjson::NO_SUCH_FIELD) {
                    data.value.template emplace<0>();
                    return false;
        ...
        template <class X>
        static bool read_simd_ondemand(X &input, std::optional<std::string> const &key, JsonVariantAutoDetect<Ts...> &data, JsonFieldMapping const &mapping=JsonFieldMapping {}) {
            if (key) {
                auto x = input.get_object()[*key];
                if (x.error() == simdjson::NO_SUCH_FIELD) {
                    data.value.template emplace<0>();
                ...
                if constexpr (IsRemainingFieldHandler<F>::value) {
                    ...
                    if (s_fieldNameMappingFilled && mappingForThisOne == nullptr) {
                        ...
                        if (input.find(s) == input.end()) {
                            if constexpr (IsJsonRequiredField<F>) {
                                return false;
                            }
                            return read_impl<FieldCount,FieldIndex+1>(input, data, mapping, mappingForThisOne, retSoFar);
                        } else {
